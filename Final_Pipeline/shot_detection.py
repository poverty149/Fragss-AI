...
def process_frames(video_path, start_frame, end_frame, step_size, min_frames=50, max_frames=200, method='hist'):
    cap = cv2.VideoCapture(video_path)
    ...
    if method == 'hist':
        ...
        while frame_number < end_frame:
            ...
            if prev_hist is not None:
                ...
                if diff < 0.7 or (frame_number - prev_bound) >= max_frames:
        ...
        while frame_number < end_frame:
            ...
            if prev_des is not None:
                ...
                if len(good_matches) < match_threshold or (frame_number - prev_bound) >= max_frames:
...
def parallel_frame_processing(video_path, num_threads=4,min_clip_length=5,max_clip_length=20):
    cap = cv2.VideoCapture(video_path)
    ...
    max_frames=fps*max_clip_length
    ...
    with ThreadPoolExecutor(max_workers=num_threads) as executor:
        for i in range(num_threads):
            ...
            futures.append(executor.submit(process_frames, video_path, start_frame, end_frame, step_size,min_frames,max_frames))
...
def save_clips_using_processed_frames(frames, shot_boundaries, video_path, output_dir="static/clips"):
    cap = cv2.VideoCapture(video_path)
...
def save_clips_using_processed_frames_with_audio(frames, shot_boundaries, video_path, output_dir="static/clips"):
    cap = cv2.VideoCapture(video_path)
    fps = cap.get(cv2.CAP_PROP_FPS)
    width = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
    height = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
